...
public class PMerge implements Callable<int[]> {
    ...
    public static void parallelMerge(int[] A, int[] B, int[]C, int numThreads) {
        ...
        try {
            ...
            for (int i = 0; i < A.length; i += num_tasks_a) {
                if (i + num_tasks_a > A.length) { // handle extra elements / un-even split
                    PMerge pm = new PMerge(A, B, C, i, A.length - 1, true);
                    Future<int[]> pm1 = es.submit(pm);
                    pm1.get();
                } else {
                    PMerge pm = new PMerge(A, B, C, i, i + (num_tasks_a - 1), true);
                    Future<int[]> pm1 = es.submit(pm);
                    pm1.get();
                }
            }
            ...
            for (int i = 0; i < B.length; i+=num_tasks_b) {
                if (i + num_tasks_b > B.length) { // handle extra elements / un-even split
                    PMerge pm = new PMerge(A, B, C, i, B.length - 1, false);
                    Future<int[]> pm1 = es.submit(pm);
                    pm1.get();
                } else {
                    PMerge pm = new PMerge(A, B, C, i, i + (num_tasks_b - 1), false);
                    Future<int[]> pm1 = es.submit(pm);
                    pm1.get();
                }
